...
// to use,
// add a click listener for
// requestPermission() (for safari)
// then call
// wakeLock.init();
...
let wakeLock = {
  ...
  init: function() {
    ...
    document.addEventListener('fullscreenchange', wakeLock.handleVisibilityChange);
    
  },
  
  handleVisibilityChange: function() {
    
    if (wakeLock.lock !== null && document.visibilityState === 'visible') {
  ...
  // on Safari
  initBackupLock: function() {

    // setup no sleep video element
    // https://github.com/richtr/NoSleep.js
    
    wakeLock.noSleepVideo = document.createElement('video');

    wakeLock.noSleepVideo.setAttribute('title', 'No Sleep');
    wakeLock.noSleepVideo.setAttribute('playsinline', '');

    wakeLock.utils.addSourceToVideo(wakeLock.noSleepVideo, 'webm', wakeLock.utils.webmVideo);
    wakeLock.utils.addSourceToVideo(wakeLock.noSleepVideo, 'mp4', wakeLock.utils.mp4Video);

    wakeLock.noSleepVideo.addEventListener('loadedmetadata', () => {
      
      if (wakeLock.noSleepVideo.duration <= 1) {
        
        // webm source
        wakeLock.noSleepVideo.setAttribute('loop', '');
        
      } else {
        
        // mp4 source
        wakeLock.noSleepVideo.addEventListener('timeupdate', () => {
          
          if (wakeLock.noSleepVideo.currentTime > 0.5) {
            wakeLock.noSleepVideo.currentTime = Math.random();
          }
  ...
  // request backup lock permission (on Safari)
  requestPermission: async function() {
    ...
    if (!wakeLock.supported) {
      ...
      try {
        ...
        await wakeLock.noSleepVideo.play();
